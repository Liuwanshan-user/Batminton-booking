...
SELECT_COURT_JS = r"""
var timeText = arguments[0] || '';
var courtIndex = parseInt(arguments[1] || 1, 10);
var done = arguments[arguments.length - 1];
...
function tryClick(el) {
  ...
  for (var depth = 0; depth < 4; depth += 1) {
    ...
    if (['button', 'a', 'li', 'div', 'span', 'label'].indexOf(tag) !== -1) {
      break;
    }
    candidate = candidate.parentElement;
  }
  if (!candidate) candidate = el;
  try { candidate.scrollIntoView({ block: 'center', behavior: 'instant' }); } catch (err) {}
  try { candidate.click(); return true; } catch (err) {}
  try {
...
var keywords = [];
if (timeText) {
  keywords.push(norm(timeText));
  keywords.push(norm(timeText.replace(/[:：]/g, '')));
}
if (courtIndex > 0) {
  var idx = String(courtIndex);
  keywords.push(norm('第' + idx));
  keywords.push(norm(idx + '号'));
  keywords.push(norm('场地' + idx));
  keywords.push(norm('球场' + idx));
  keywords.push(norm('第' + idx + '块'));
  keywords.push(norm('第' + idx + '场'));
  keywords.push(norm('court' + idx));
  keywords.push(norm('field' + idx));
  keywords.push(norm(idx));
}

var containers = Array.from(document.querySelectorAll(
  '.el-dialog, .dialog, .reservation, aside, .right, .order, .order-box, .order-info, .orderDetail, .detail, .confirm, .confirm-box'
));
...
function hasActiveSelection() {
  var selectors = ['.active', '.is-active', '.selected', '.is-selected', '.checked', '.cur', '.current'];
  return containers.some(function (container) {
    return selectors.some(function (sel) {
      var nodes = container.querySelectorAll(sel);
      return Array.from(nodes).some(function (node) { return isVisible(node); });
    });
  });
}

if (hasActiveSelection()) {
  finish({ status: 'ALREADY_SELECTED' });
  return;
}

function matchKeywords(el) {
  if (!el) return false;
  var parts = '';
  try { parts += '|' + (el.innerText || el.textContent || ''); } catch (err) {}
  if (el.getAttribute) {
    parts += '|' + (el.getAttribute('title') || '');
    parts += '|' + (el.getAttribute('data-title') || '');
    parts += '|' + (el.getAttribute('data-name') || '');
    parts += '|' + (el.getAttribute('data-value') || '');
    parts += '|' + (el.getAttribute('aria-label') || '');
    parts += '|' + (el.getAttribute('data-index') || '');
    parts += '|' + (el.id || '');
  }
  var normalized = norm(parts);
  if (!normalized) return false;
  return keywords.some(function (kw) { return kw && normalized.indexOf(kw) !== -1; });
}

var keywordCandidates = [];
containers.forEach(function (container) {
  var nodes = container.querySelectorAll('[data-index], [data-value], [data-name], [data-title], .site, .field, .order-item, .court, .room, .item, li, button, div, span, a, label');
  Array.from(nodes).forEach(function (node) {
    if (!isVisible(node)) return;
    if (matchKeywords(node)) {
      keywordCandidates.push(node);
    }
  });
});

for (var i = 0; i < keywordCandidates.length; i += 1) {
  var target = keywordCandidates[i];
  if (tryClick(target)) {
    finish({ status: 'CLICKED', text: (target.innerText || target.textContent || '').trim().slice(0, 60) });
    return;
  }
}
...
for (var c = 0; c < containers.length; c += 1) {
  ...
  var nodes = container.querySelectorAll('button, li, div, span, a, label');
  for (var j = 0; j < nodes.length; j += 1) {
    var node = nodes[j];
    if (!isVisible(node)) continue;
    var text = '';
    try { text = (node.innerText || node.textContent || '').trim(); } catch (err) {}
    if (!text) continue;
    if (/场地|球场|球馆|court|field/.test(text)) {
      if (tryClick(node)) {
        finish({ status: 'FALLBACK_CLICKED', text: text.slice(0, 60) });
        return;
      }
    }
  }
...
def _click_selected_court_icon_dom(
    driver, time_text: str, court_index: int, timeout: float = 2.5
) -> bool:
    ...
    while time.time() < end_time:
        ...
        time.sleep(0.2)
    log("⚠ DOM 遍历未能点击已选场地图标，尝试使用 JS 辅助。")
    return False


def _click_selected_court_icon_js(driver, time_text: str, court_index: int) -> bool:
    try:
        raw = driver.execute_async_script(SELECT_COURT_JS, time_text, court_index)
    except Exception as exc:
        log(f"JS 选择场地图标失败：{exc}")
        return False
    ...
    if isinstance(raw, str):
        try:
            ...
            log(f"解析 JS 返回值失败：{raw!r} ({exc})")
            return False
    elif isinstance(raw, dict):
        res = raw
    else:
        log(f"JS 返回未知类型：{raw!r}")
        return False

    status = res.get("status")
    text = res.get("text")
    if status in {"CLICKED", "FALLBACK_CLICKED"}:
        log(
            "JS 已点击场地：{} ({})".format(
                text or f"第{court_index}块", status
            )
        )
        return True
    ...
    log(f"JS 未能选中场地，状态：{status} 返回：{res}")
    return False


def click_selected_court_icon(
    driver, time_text: str, court_index: int, timeout: float = 2.5
) -> bool:
    if _click_selected_court_icon_dom(driver, time_text, court_index, timeout=timeout):
        return True
    return _click_selected_court_icon_js(driver, time_text, court_index)
