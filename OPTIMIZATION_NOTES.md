...
### 2025-10-31 修复误判问题
**问题**：快速检查功能有时会将有可用场地的时间段误判为0个可用场地

**修复措施**：
1. ✅ **添加详细调试日志**：输出完整的DOM结构分析信息、样本class名称
2. ✅ **修复JS bug**：变量名 `isVisible` 与函数名冲突，改为 `seatVisible`
3. ✅ **保守策略**：未知状态的可见座位也计入可用（防止遗漏）
4. ✅ **防御性重试**：即使快速检查显示0个可用，仍会尝试第一个场地（除非确定全部已订）
5. ✅ **智能跳过**：只有当 `已订数量 = 总座位数` 时才确定跳过

**日志输出示例**：
```
🔍 快速检查 18:00: 找到15个时间项 | ✓ 找到时间索引:3 | 找到15个座位行 | 该行共12个座位 | 统计: 可用=3, 已订=9, 未知=0
   样本class: ['inner-seat unselected-seat', 'inner-seat bought-seat', 'inner-seat unselected-seat']
✓ 时间段 18:00 发现 3/12 个可用场地，开始抢订
```

如果误判为0：
```
🔍 快速检查 19:00: 找到15个时间项 | ✓ 找到时间索引:4 | 找到15个座位行 | 该行共12个座位 | 统计: 可用=0, 已订=10, 未知=2
   样本class: ['inner-seat some-unknown-class', 'inner-seat bought-seat', ...]
⚠️ 时间段 19:00 快速检查显示无可用场地，但仍会尝试（可能误判）
```
